...
type TestCases = [
  ...
  // if `{id: true}` is equivalent to `{ id: true; posts: { select: undefined } }`
  ExpectTrue<
    Equals<{ id: number; posts: never }, Prisma.UserGetPayload<{ select: { id: true; posts: { select: undefined } } }>>
  >,
  // else if runtime behavior is intended
  ExpectFalse<
    Equals<
      {
        id: number
